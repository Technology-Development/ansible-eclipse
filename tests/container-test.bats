...
@test "Role can be applied to container" {
  ...
  docker_exec_q test -f /usr/local/eclipse-platform-4.5.2-linux-gtk-x86_64/eclipse
  docker_exec alternatives --display eclipse | grep "/usr/local/eclipse-platform-4.5.2-linux-gtk-x86_64"
}

@test "Override default eclipse url" {
  ansible-playbook -i hosts --extra-vars "eclipse_url=http://www.eclipse.org/downloads/download.php?file=/eclipse/downloads/drops4/R-4.5.2-201602121500/eclipse-platform-4.5.2-linux-gtk.tar.gz&r=1" test.yml
  docker_exec_q test -f /usr/local/eclipse-platform-4.5.2-linux-gtk/eclipse
  docker_exec alternatives --display eclipse | grep "/usr/local/eclipse-platform-4.5.2-linux-gtk"
}
